...
def lbe(pvals, a=None, lb=0.05, ci_level=0.95, qvalues=True,
        fdr_level=0.05, n_significant=None):
    ...
    if min(pvals) < 0 or max(pvals) > 1:
        ...
        if qvalues:
            ...
            for i in range(2, m+1):
                qval[m - i] = min((pi0 * m * sort_pval[m - i])/(m - i),
                                  qval[m - i + 1])
        ...
        if sdbound > 0.5:
            print("""WARNING: l = {0}. A smaller value is
                    recommended for a (or l).""".format(sdbound))

        if qvalues:
            significant = (qval[rank_pval] <= fdr_level)
            r = [fdr, pi0, icpi0, ci_level, a, sdbound, qval[rank_pval], pvals,
                 significant, (significant[significant == True]).sum()]
        else:
            r = [None, pi0, icpi0, ci_level, a, sdbound,
                 None, pvals, None, None]

    return r


def lbe_a(m, l):
    aopt = max(1, minimize(asearch, [1], bounds=[(0.3, 25)], args=(m, l)).x)
    return aopt


def asearch(a, m, l):
    return np.abs(np.sqrt(1/(gamma(a + 1)) ** 2 *
                          ((gamma(2 * a + 1) - (gamma(a + 1)) ** 2)/m)) - l)
